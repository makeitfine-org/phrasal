...
describe('GetVerbPage', () => {
  ...
  it('"out" section starts expanded showing all 4 definitions', () => {
    renderPage();
    ...
    expect(screen.getByText(/To produce or publish something/i)).toBeInTheDocument();
    expect(screen.getByText(/To avoid doing something you do not want to do/i)).toBeInTheDocument();
  });

  it('clicking "out" collapses all "out" meaning cards', () => {
    renderPage();
  ...
  it('clicking "out" twice restores all "out" meaning cards', () => {
    renderPage();
    fireEvent.click(screen.getByText('out'));
    fireEvent.click(screen.getByText('out'));
    expect(screen.getByText(/To leave a place or a car/i)).toBeInTheDocument();
  ...
  it('saves "out" section state to localStorage when collapsed', () => {
    renderPage();
    fireEvent.click(screen.getByText('out'));
    expect(localStorage.getItem('getOut_section_expanded')).toBe('false');
  });
